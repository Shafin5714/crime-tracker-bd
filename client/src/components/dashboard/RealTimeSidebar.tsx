...
import { AlertCircle, Plus, Users, Eye, Clock, MapPin, ChevronLeft, ChevronRight } from "lucide-react";
...
export function RealTimeSidebar({ isOpen, onToggle }: RealTimeSidebarProps) {
  return (
    <aside className={`relative flex-shrink-0 transition-all duration-300 z-20 ${isOpen ? "w-80" : "w-0"}`}>
      {/* Sidebar Content Container */}
      <div
        className={`w-80 h-full border-l bg-background flex flex-col overflow-hidden transition-opacity duration-200 ${
      ...
      <Button
        ...
        size="icon"
        onClick={onToggle}
        className="absolute -left-3 top-6 z-50 h-6 w-6 rounded-full border shadow-md p-0"
      >
